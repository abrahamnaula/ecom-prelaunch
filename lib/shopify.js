...
export async function PredictiveProductSearch(queryString) {
    const query = `
  ...
  predictiveSearch(query: $query) {
    ...
    try {
        ...
        if (response.errors || !response.data.predictiveSearch) {
            throw new Error('Failed to perform predictive search');
        }

        return response.data.predictiveSearch.products;
    } catch (error) {
