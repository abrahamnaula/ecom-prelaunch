...
export async function getCollectionCursors(handle) {
    ...
    while(hasNextPage) {
        ...
        // Get the cursors and add to the list
        const edges = res.data.collection.products.edges;
        const cursorsInPage = edges.map(edge => edge.cursor);
        cursors = [...cursors, ...cursorsInPage];

        hasNextPage = res.data.collection.products.pageInfo.hasNextPage;
        lastCursor = cursorsInPage[cursorsInPage.length - 1];
    }
